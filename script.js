...
(function () {
  ...
  (async function run() {
    try {
      // Allow pages to opt-out from auto-loading posts (e.g., research.html uses its own renderer)
      const autoload = (document.body && document.body.getAttribute && document.body.getAttribute('data-autoload-posts')) || '';
      if (String(autoload).toLowerCase() === 'false') {
        return; // don't autoload here, but keep injected styles/utilities
      }
